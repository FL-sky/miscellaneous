...
public:
    ...
    //
    //
    int dfs1(int u, int delta, int si)
    {
        if (u == sink)
            return delta;
        int ret = 0;
        if (u == src)
        {
            ...
            if (e[i].f && dist[e[i].v] == dist[u] + 1)
            {
                int dd = dfs1(e[i].v, min(e[i].f, delta), si);
                e[i].f -= dd;
                e[i ^ 1].f += dd;
                delta -= dd;
                ret += dd;
    ...
    //
    //
    int dfs2(int u, int delta, int lim, int ci) //弱化某点
    {
        if (u == sink)
            return delta;
        int ret = 0;
        if (u == src)
        {
            ...
            for (int ik = 0; ik < tv.size(); ik++)
            {
                ...
                if (e[i].f && dist[e[i].v] == dist[u] + 1 && e[i ^ 1].f < lim)
                {
                    int dd = dfs2(e[i].v, min(lim, min(e[i].f, delta)), lim, ci);
                    e[i].f -= dd;
                    e[i ^ 1].f += dd;
                    delta -= dd;
                    ret += dd;
        ...
        else
        {
            for (int i = g[u]; i; i = e[i].nxt)
            {
                if (e[i].f && dist[e[i].v] == dist[u] + 1)
                {
                    int dd = dfs2(e[i].v, min(e[i].f, delta), lim, ci);
                    e[i].f -= dd;
                    e[i ^ 1].f += dd;
                    delta -= dd;
                    ret += dd;
...
void solv()
{
    ...
    for (int it = 0; it < demand.size(); it++)
    {
        ...
        iter = it;
        int mf = 0, md = 40;
        if (it % md && server.size() < md)
            mf = mcf.maxflow(sum);
        else
        {
            int si = (++sid) % server.size();
            mf = mcf.maxflow1(sum, si);
            mf += mcf.maxflow2(sum, si);
        }
        assert(mf == sum);
        bool ck = true;
        int ussv[135];
